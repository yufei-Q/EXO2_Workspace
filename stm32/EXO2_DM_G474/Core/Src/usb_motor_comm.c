...
#define USB_COMMAND_PAYLOAD_SIZE        \
  (USB_MOTOR_COMMAND_SIZE * DM_MOTOR_COUNT)
#define USB_FEEDBACK_PAYLOAD_SIZE       \
  (USB_MOTOR_FEEDBACK_SIZE * DM_MOTOR_COUNT)
...
{
  DM_MotorFeedback_t feedback[DM_MOTOR_COUNT];
  uint8_t *payload;
  uint16_t crc;
  uint32_t interrupt_state;
  uint8_t index;
  ...
  for (index = 0U; index < DM_MOTOR_COUNT; ++index)
  {
    feedback[index] = dm_motors[index].feedback;
  }
  if (interrupt_state == 0U)
  {
    __enable_irq();
  ...
  crc = USB_MotorComm_Crc16(&s_tx_frame[2],
                            (uint16_t)(USB_FEEDBACK_PAYLOAD_SIZE + 5U));
  USB_MotorComm_WriteUint16(
    &s_tx_frame[USB_FRAME_PAYLOAD_OFFSET + USB_FEEDBACK_PAYLOAD_SIZE], crc);
